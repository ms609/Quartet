...
test_that("CPDT produces correct results with four leaves", {
  tree4.1 <- ape::read.tree(text = "(1, (2, (3, 4)));")
  tree4.2 <- ape::read.tree(text = "(1, (4, (3, 2)));")
  expect_equal(CPDTDist(tree4.1, tree4.1), 0L)
  expect_equal(CPDTDist(tree4.2, tree4.2), 0L)
...
test_that("CPDT produces sane results", {
  ...
  rtree1 <- RootTree(BalancedTree(8), "t1")
  tree2 <- PectinateTree(8)

  expect_equal(CPDTDist(rtree1, rtree1), 0L)
