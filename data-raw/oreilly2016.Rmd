...
ReadNexusSafely <- function(filename) {
  if (file.exists(filename)) read.nexus(filename) else NULL
}
...
#data("orQuartets")
if (!exists("orPartitions")) {
  BLANK_SPLITS <- array(NA, c(6, 21, length(FILE_NUMS)),
                        dimnames=list(c("cf", "ref", "cf_and_ref",
...
#data("orPartitions")
if (!exists("orQuartets")) {
  BLANK_QUARTS <- array(NA, c(6, 21, length(FILE_NUMS)),
                        dimnames=list(c( "Q",  "s", "d", "r1", "r2", "u"),
                                      NULL, FILE_NUMS))
...
THESE_FILE_NUMS <- paste0(1:100, '_1')

for (NCHAR in c(100, 1000)) { # TODO restore 350, 
