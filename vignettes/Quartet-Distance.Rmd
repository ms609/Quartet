...
to distinguish between trees [@Steel1993].
It also encounters artefacts and biases in a range of situations,
limiting its performance against a suite of real-world benchmarks 
[@Smith2020;@Smith2021]; generalizations of the Robinson-Foulds distance
(see R package '[TreeDist](https://ms609.github.io/TreeDist/)')
go some way to mitigating these shortcomings.
...
'reference'.
Examples might include a tree validated by independent data, or a tree used 
to generate data from which further trees have been reconstructed.
...
The below code calculates the mean proportion of matching quartets for 20 random 
trees with 4 to 20 leaves, and the corresponding standard deviation.

```{r mean-proportion}
round(vapply(4:20, function (nTip) {
 trees <- lapply(rep(nTip, 21), TreeTools::RandomTree)
 results <- QuartetStatus(trees)[1, ] / choose(nTip, 4)
