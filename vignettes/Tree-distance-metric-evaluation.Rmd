...
```{R initialize, echo=FALSE, meassage=FALSE, warning=FALSE}
library('TreeSearch')
#unloadNamespace('TreeSearch')
##devtools::load_all('c:/research/r/TreeSearch')
unloadNamespace('Quartet')
##library('Quartet')
library('Quartet')
...
ColPlot <- function (tr, title=NULL, bold=NULL, ...) {
  ...
  plot(tr, tip.col=Ternary::cbPalette15[-c(4, 7)][as.integer(tr$tip.label)], 
       ...
       y.lim=c(-9.5, 11), ...) ## Change this when adding to legend
}
...
RFPlot <- function (tr, title=NULL, highlight=NULL, ref=sq_trees[[1]], 
  ...
  pair_info <- signif(TreeSearch::PartitionInfo(tr) + TreeSearch::PartitionInfo(ref), 4)
  ...
  y <- 0.5
  text(text_x, y <- y - 1, "VoArInf:", cex=0.8, pos=2)
  text(text_x, y <- y - 1, "VoParInf:", cex=0.8, pos=2)
  text(text_x, y <- y - 1, "VoParInf+:", cex=0.8, pos=2)
  
  text(text_x, y <- y - 1, "NyeEtAl:", cex=0.8, pos=2)
  text(text_x, y <- y - 1, "MS:", cex=0.8, pos=2)
  
  text(text_x, y <- y - 1, 'Quartet:', cex=0.8, pos=2)
  text(text_x, y <- y - 1, "Path:", cex=0.8, pos=2)
  text(text_x, y <- y - 1, "SPR:", cex=0.8, pos=2)
  text(text_x, y <- y - 1, "RF:", cex=0.8, pos=2)
  #text(text_x, y <- y - 1, "VPI%:", cex=0.8, pos=2)
  
  y <- 0.5
  text(text_x, y <- y - 1, paste0(
    signif(TreeSearch::VariationOfArborealInfo(tr, ref), 4), ' / ', pair_info
    ), cex=0.8, pos=4)
  text(text_x, y <- y - 1, paste0(signif(TreeSearch::VariationOfPartitionInfo(tr, ref), 4),
                                  ' / ', pair_info), cex=0.8, pos=4)
  text(text_x, y <- y - 1, paste0(signif(TreeSearch::VariationOfPartitionInfo(tr, ref, MAX=sum), 4), ' / ', pair_info), cex=0.8, pos=4)
  
  text(text_x, y <- y - 1, paste0(-signif(TreeSearch::NyeTreeSimilarity(tr, ref), 4), '/8'), cex=0.8, pos=4)
  text(text_x, y <- y - 1, paste0(signif(TreeSearch::MatchingSplitDistance(tr, ref), 4),
                                  '/32'), cex=0.8, pos=4)
  ...
  text(text_x, y <- y - 1, paste0(tree_dist['symmetric.difference'], '/', (11L - 3L) * 2L), cex=0.8, pos=4)
  #text(text_x, y <- y - 1, paste0(100 * signif(TreeSearch::VariationOfPartitionInfo(tr, ref) / pair_info, 4), '%'), cex=0.8, pos=4)
}
```
...
| Measure | Quartet | Path    | SPR     | RF      | MS  | Nye _et al._ | MClInf  | MArInf  | VoArInf |
| ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | 
| Outcome | `r pass` | `r pass` | `r FAIL` | `r FAIL` | `r pass` | `r pass` | `r pass` | `r pass` | `r pass` |
...
```{R Cherry-short-move, echo=FALSE, message=FALSE, cache=TRUE, fig.width=6, fig.height=4, out.width='66%', fig.align='center'}
...
Arguably this is an ambiguous case, as the moves are equivalent to moving (10, 11) by one node; perhaps the quartet metric can be forgiven for allocating both moves an equivalent score.  With longer moves, all metrics except the SPR, RF and MS allocate higher scores when larger clades are moved:

```{R Cherry-short-move-2, echo=FALSE, message=FALSE, cache=TRUE, fig.width=6, fig.height=4, out.width='66%', fig.align='center'}
par(mfrow=c(1, 3), mar=c(2.4, 0.4, 0.4, 0.4), cex=1)
...
The Nye _et al_. metric also fails this test under certain situations:

```{R Move-a-cherry-two, echo=FALSE, message=FALSE, cache=TRUE, fig.width=6, fig.height=4, out.width='66%', fig.align='center', eval=FALSE}
par(mfrow=c(1, 3), mar=c(2.4, 0.4, 0.4, 0.4), cex=1)
...
| Measure | Quartet | Path    | SPR     | RF      | MS  | Nye _et al._ | MClInf  | MArInf  | VoArInf |
| ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | 
| Short move | `r pass` | `r pass` | `r FAIL` | `r FAIL` | `r pass` | `r pass` | `r pass` | `r pass` | `r pass` |
| Medium move | `r pass` | `r pass` | `r FAIL` | `r FAIL` | `r FAIL` | `r FAIL` | `r pass` | `r pass` | `r pass` |
| Long move | `r pass` | `r pass` | `r FAIL` | `r FAIL` | `r FAIL` | `r FAIL` | `r pass` | `r pass` | `r pass` |

## Moving multiple taxa

```{r simple-vs-complex, echo=FALSE, cache=TRUE, warn=FALSE, message=FALSE, fig.height=6, fig.width=5.6, out.width='80%', fig.align='center'}
par(mfrow=c(1, 3), mar=c(2.4, 0.4, 0.4, 0.4), cex=1)
...
#RFPlot(ape::read.tree(text='(((((1, 5), 6), 2), (3, 4)), ((8, (7, 9)), (10, 11)));'), 'Short shuffle', ref=pectinate_tree)
#RFPlot(ape::read.tree(text='(1, (2, ((3, (4, 5)), ((8, 9), (7, (11, (6, 10)))))));'), 'Discovered: move 5 (4,5,8,9,6)', ref=pectinate_tree)
RFPlot(ape::read.tree(text='(1, ((2, (4, 5)), (3, ((8, 9), ((7, 11), (6, 10))))));'), 'Move 6: 4,5,6,8,9,11', c(4,5,6,8,9,11), ref=pectinate_tree)
...
```{R Pectinate-to-break-RF, echo=FALSE, message=FALSE, cache=TRUE, fig.width=6, fig.height=9, fig.align='center'}
par(mfrow=c(3, 2), mar=rep(0.4, 4), cex=1)
remove(sq_trees)
data('sq_trees')
...
RFPlot(pectinate_move1, 'Move one taxon', 1, ref=pectinate_tree, edge.lengths = c(rep(1, 4), 2, rep(1, 12), .5, .5, 1.5))
#ape::edgelabels()
RFPlot(sq_trees$top_and_tail, 'Swap two taxa', c(1, 11), ref=pectinate_tree)
...
| Criterion       | Quartet | Path    | SPR     | RF      | MS  | Nye _et al._ | MClInf  | MArInf  | VoArInf |
| ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- | 
| Swap 2 > Move 1 | `r pass` | `r pass` | `r pass` | `r FAIL` | `r FAIL` | `r pass` | `r pass` | `r pass` | `r pass` |
| Random > Swap 2 | `r pass` | `r pass` | `r pass` | `r FAIL` | `r pass` | `r pass` | `r pass` | `r FAIL` | `r FAIL` |
| Contra > Random | `r pass` | `r pass` | `r FAIL` | `r FAIL` | `r pass` | `r pass` | `r pass` | `r pass` | `r pass` |
...
random_arb_info <- TreeSearch::VariationOfArborealInfo(random_trees, pectinate_tree)
pect_arb_info <- TreeSearch::VariationOfArborealInfo(pectinate_move1, pectinate_tree)
random_par_info <- TreeSearch::VariationOfPartitionInfo(random_trees, pectinate_tree)
pect_par_info <- TreeSearch::VariationOfPartitionInfo(pectinate_move1, pectinate_tree)
random_parsum_info <- TreeSearch::VariationOfPartitionInfo(random_trees, pectinate_tree, MAX=sum)
pect_parsum_info <- TreeSearch::VariationOfPartitionInfo(pectinate_move1, pectinate_tree, MAX=sum)
...
PlotHist(random_distances['symmetric.difference', ] / 16, manyBreaks, 1, 'Robinson-Foulds distance')
mtext('Distance between pectinate tree (above) and random bifurcating trees', outer=TRUE, cex=0.8)

PlotHist(random_arb_info, max(random_arb_info, pect_arb_info) * manyBreaks, pect_arb_info, 'Var of Arb Info')

PlotHist(random_distances['path.difference', ], breaks=manyBreaks * max(random_distances['path.difference', ]),
         17.7, "Path distance")

PlotHist(random_par_info, max(random_par_info, pect_par_info) * manyBreaks, pect_par_info, 'Var of Par Info')
PlotHist(random_parsum_info, max(random_parsum_info, pect_parsum_info) * manyBreaks, pect_parsum_info, 'Var of ParSUM Info')
