...
RFPlot <- function (tr, title=NULL, highlight=NULL, ref=sq_trees[[1]], 
  ...
  #text(text_x, y <- y - 1, "VoParInf+:", cex=0.8, pos=2)
  text(text_x, y <- y - 1, "CluInf:", cex=0.8, pos=2)
  ...
  text(text_x, y <- y - 1, paste0(signif(TreeSearch::VariationOfClusteringInfo(tr, ref), 4),
                                  ' / ',
                                  signif(TreeSearch::ClusteringInfo(tr) +
                                           TreeSearch::ClusteringInfo(ref), 4)),
       cex=0.8, pos=4)
...
pect_arb_info <- TreeSearch::VariationOfArborealInfo(pectinate_move1, pectinate_tree)
random_clu_info <- TreeSearch::VariationOfClusteringInfo(random_trees, pectinate_tree)
pect_clu_info <- TreeSearch::VariationOfClusteringInfo(pectinate_move1, pectinate_tree)
...
PlotHist(random_par_info, max(random_par_info, pect_par_info) * manyBreaks, pect_par_info, 'Var of Partition Info')
PlotHist(random_clu_info, max(random_clu_info, pect_clu_info) * manyBreaks, pect_clu_info, 'Var of Clustering Info')
