---
title: "Tree distance metric evaluation"
...
unloadNamespace('TreeDist')
devtools::load_all('c:/research/r/TreeTools')
devtools::load_all('c:/research/r/TreeDist')
library('Quartet')
...
RFPlot <- function (tr, title=NULL, highlight=NULL, ref=sq_trees[[1]], ...) {
  ...
  text(text_x, y <- y - 1, "MClInf:", cex=0.8, pos=2)
  text(text_x, y <- y - 1, "MArInf:", cex=0.8, pos=2)
  text(text_x, y <- y - 1, "VoArInf:", cex=0.8, pos=2)
  
  y = 0.5
  text(text_x, y <- y - 1, paste0(QuartetStatus(list(tr, ref))[2, 'd'],
                            '/', choose(11,4)), cex=0.8, pos=4)
  text(text_x, y <- y - 1, paste0(signif(tree_dist[2], 3)), cex=0.8, pos=4)
  ...
  text(text_x, y <- y - 1, signif(TreeDist::MutualClusterInfo(tr, ref), 4), cex=0.8, pos=4)
  text(text_x, y <- y - 1, signif(TreeDist::MutualArborealInfo(tr, ref), 4), cex=0.8, pos=4)
  text(text_x, y <- y - 1, signif(TreeDist::VariationOfArborealInfo(tr, ref), 4), cex=0.8, pos=4)
}
...
```{R Moving-a-cherry, echo=FALSE, message=FALSE, cache=TRUE, fig.width=6, fig.height=12, out.width='66%', fig.align='center'}
par(mfrow=c(3, 3), mar=c(2.4, 0.4, 0.4, 0.4), cex=1)

RFPlot(sq_trees[[1]], 'Reference tree')
...
plot.new()
RFPlot(pectinate_move1, 'Move one taxon', 1, ref=pectinate_tree)
...
RFPlot(sq_trees$anti_pectinate, 'Random contradictory tree', ref=pectinate_tree)
```
...
random_arb_info <- TreeDist::MutualArborealInfo(random_trees, pectinate_tree)
random_clu_info <- TreeDist::MutualClusterInfo(random_trees, pectinate_tree)
...
pect_arb_info <- TreeDist::MutualArborealInfo(pectinate_move1, pectinate_tree)
...
pect_clu_info <- TreeDist::MutualClusterInfo(pectinate_move1, pectinate_tree)
