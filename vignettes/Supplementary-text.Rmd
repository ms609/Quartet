...
A further shortcoming of the partition metric is that not all partitions are equivalent.
...
plot(balancedTree <- read.tree(text="(((a, b), (c, d)), ((e, f), (g, h)));"),
     edge.color = cbPalette8[2]); edgelabels(1:5, c(1, 2, 5, 9, 12))
plot(caterpillarTree <- read.tree(text="(a, (b, (c, (d, (e, (f, (g, h)))))));"),
     edge.color = cbPalette8[3]); edgelabels(1:5, c(4, 6, 8, 10, 12))
```
...
names(matchingTrees) <- paste0(splitSmall, ':', splitLarge)
...
In the first tree, split 1 is even, dividing four taxa from four others; splits 2--5 are maximally uneven.  The total information content of these five splits is `r signif(sum(ic[c(4, 2, 2, 2, 2) - 1]), 4)`, whereas that of the five splits in 
the second tree, which resepctively split 2, 3, 4, 3 and 2 taxa from the rest, is
...
Of the `r 11*9*7*5*3` 8-tip trees, many more bear at least one partition in 
common with a balanced tree than with an asymmetric tree:

```{r all-8-tip-trees, echo=FALSE, cache=TRUE, results='asis'}
calculate <- FALSE
if (calculate) {
  ...
  balancedSplitCount <- table(inBalanced)
  caterpillarSplitCount <- table(inCaterpillar)
  balancedQuartetCount <- table(qInBalanced)
  caterpillarQuartetCount <- table(qInCaterpillar)
} else {
  balancedSplitCount <- c(7088, 2708, 512, 76, 10, 1)
  caterpillarSplitCount <- c(8162, 1808, 350, 64, 10, 1)
  names(balancedSplitCount) <- names(caterpillarSplitCount) <- 0:5
}

par(mfrow=c(1, 2))
hist(balancedSplitCount)
hist(caterpillarSplitCount, add=TRUE)

#knitr::kable(cbind('Partitions shared with balanced tree' = balancedSplitCount,
#                   'Partitions shared with asymmetric tree' = caterpillarSplitCount),
#             caption = 'Trees with N partitions in common')
#
#knitr::kable(cbind('Quartets shared with balanced tree' = balancedQuartetCount,
#                   'Quartets shared with asymmetric tree' = caterpillarQuartetCount),
#             caption = 'Trees with N quartets in common')
