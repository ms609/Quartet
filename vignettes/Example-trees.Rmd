...
representing a maxximal value despite retaining relationship information about 
all other taxa.

```{R Pectinate to break RF, echo=FALSE, message=FALSE, cache=TRUE, fig.asp=1.5/3, fig.width=6, fig.align='center', dev='svg'}
...
<MARK> ****TODO replace 1:10 with 1:10000**** </MARK>
...
random_trees <- lapply(1:10, function (x) rtree(11, br=NULL, tip.label=1:11))
...
TreeSearch` [@TreeSearch], using the quartet distance from the reference tree as an optimality criterion.
...
The _y_ axis of these plots corresponds to the overall quartet divergence / normalized symmetric distance metrics.   Collapsing nodes decreases the resolution (movement in the _x_ direction), but can increase accuracy, leading to no net decrease in tree divergence.
...
Under the partition metric, the most informative trees were found after collapsing nodes with a Bremner support value of:
