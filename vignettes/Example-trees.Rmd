...
proportion agree -- the strict joint assertions (_SJA_) measaure.
...
$Q - ((2d + r_1 + r_2) / 2Q)$.  (An equivalent calculation underlies the
partition metric.)
...
legend('bottomright', cex=0.8, bty='n',
       pch=c(20, pchs, rep(20, 5)), pt.cex=1.5,
       col=c('white', rep('black', 3), rep('white', 2), cbPalette8[2:4]),
       legend=c('Collapse:', 'No nodes', 'One node', 'Many nodes',
      '', 'Move:', 'No tips', 'One tip', 'Two tips'))


```

This provides an interesting way to explore how a dataset responds to blablabla
An intelligent approach to [@Congreve2016] 
Congreve & Lamsdell argue that the 'best' trees are those that are least resolved
Well I'm able to suggest otherwise
Look what happens when you reduce resolution.

```{R Congreve Lamsdell tree, fig.height=3, fig.width=9}
...
COL_MK <- rgb(0.5, 0.5, 0.5)
COL_EQ <- rgb(0.2, 0.2, 0.8)
COL_1  <- rgb(0.5, 0.1, 0.5)
COL_2  <- rgb(0.6, 0.2, 0.3)
COL_3  <- rgb(0.7, 0.4, 0.4)
COL_5  <- rgb(0.8, 0.6, 0.1)
COL10  <- rgb(0.9, 0.8, 0.0)

Quartet2Ternary <- function (item, i) clQuartets[[item]][c('s', 'd', 'r2'), , i]
  
Split2Ternary <- function (item, i) {
  itemData <- clPartitions[[item]][, , i]
  rbind(itemData['cf_and_ref', ],
        itemData['cf_not_ref', ],
        itemData['ref', ] - itemData['cf', ])
}

TernaryQuarts<-function(xLim, yLim, padding=0.1) {
  TernaryPlot(' Consistent', ' Contradicted', 'Unresolved', lab.cex=0.8, lab.font=2,
              grid.lty='solid', col=rgb(0.9, 0.9, 0.9), grid.col='white', 
              grid.lines=7,  axis.labels = round(seq(0, choose(22, 4), length.out=8), 0),
              axis.col=rgb(0.6, 0.6, 0.6), ticks.col=rgb(0.6, 0.6, 0.6),
              padding=padding, xlim=xLim, ylim=yLim)
  HorizontalGrid()
  AddToTernary(lines, list(c(1/3, 2/3, 0), c(0, 0, 1)), lty='dotted', col=cbPalette8[8], lwd=2)
  
  JoinTheDots(Quartet2Ternary('markov'   , TREE), col=COL_MK, pch=PCH_MK, cex=1.1)
  JoinTheDots(Quartet2Ternary('equal'    , TREE), col=COL_EQ, pch=PCH_EQ, cex=1.1)
  #JoinTheDots(Quartet2Ternary('implied1' , TREE), col=COL_1,  pch=PCH_IW, cex=1.1)
  JoinTheDots(Quartet2Ternary('implied2' , TREE), col=COL_2,  pch=PCH_IW, cex=1.1)
  JoinTheDots(Quartet2Ternary('implied3' , TREE), col=COL_3,  pch=PCH_IW, cex=1.1)
  JoinTheDots(Quartet2Ternary('implied5' , TREE), col=COL_5,  pch=PCH_IW, cex=1.1)
  #JoinTheDots(Quartet2Ternary('implied10', TREE), col=COL10,  pch=PCH_IW, cex=1.1)
}


TernaryQuarts(NULL, NULL)
...
zoom <- 6
TernaryQuarts(c(0, sqrt(3/4)/zoom), c(0.5-(1/zoom), 0.5), padding=0.05)
...
JoinTheDots(Split2Ternary('markov'   , TREE), col=COL_MK, pch=PCH_MK, cex=1.1)
JoinTheDots(Split2Ternary('equal'    , TREE), col=COL_EQ, pch=PCH_EQ, cex=1.1)
#JoinTheDots(Split2Ternary('implied1' , TREE), col=COL_1, pch=PCH_IW, cex=1.1)
JoinTheDots(Split2Ternary('implied2' , TREE), col=COL_2, pch=PCH_IW, cex=1.1)
JoinTheDots(Split2Ternary('implied3' , TREE), col=COL_3, pch=PCH_IW, cex=1.1)
JoinTheDots(Split2Ternary('implied5' , TREE), col=COL_5, pch=PCH_IW, cex=1.1)
#JoinTheDots(Split2Ternary('implied10', TREE), col=COL10, pch=PCH_IW, cex=1.1)
...
legend('bottomright', cex=0.8, bty='n',
       ...
       col=c(COL_MK, COL_EQ, COL_2, COL_3, COL_5),
       legend=c('Markov', 'Equal weights', paste0('Implied, k=', c(2, 3, 5)))
)
