...
rfplot <- function (tr, title, highlight) {
  ...
  text(0,  9.6, paste0("RF = ", topo_dist), cex=0.8, pos=4)
  text(0,  8.6, paste0("Path = ", signif(tree_dist[2], 3)), cex=0.8, pos=4)
  text(0,  7.6, paste0("SPR = ", phangorn::sprdist(tr, ref_tree)[1]), cex=0.8, pos=4)
}
...
All these trees have an SPR distance of 1: they can be obtained by 
performing one subtree pruning and regrafting operation on the reference tree.

The Robinson-Foulds (RF or 'symmetric') distance [@Robinson1981] fails to distinguish
distances a medium or long move, whereas the Quartet distance identifies the
long move as representing a greater modification of the reference tree.
...
par(mfrow=c(1, 4), mar=rep(0.4, 4), cex=1)
colplot(ref_tree, 'Reference tree')
rfplot(trs$move_one_far,  'Small peturbation', 1)
rfplot(trs$random_tree,   'Random tree',       1)
rfplot(trs$opposite_tree, 'Maximum distance',  1)
```
