...
```{R Congreve Lamsdell tree, fig.height=4.5, fig.width=9, echo=FALSE}
par(mfrow=c(1, 2), mar=rep(0.3, 4))
...
TernaryQuarts(Func=AverageQuarts, zoom=4, padding=0.01)
...
```{R CL trees - partitions, echo=FALSE}
AverageSplits <- function (item) {
  itemData <- apply(clPartitions[[item]][, , 1:30], 2, rowMeans)
  rbind(itemData['cf_and_ref', ],
        itemData['cf_not_ref', ],
        itemData['ref', ] - itemData['cf', ])
}
