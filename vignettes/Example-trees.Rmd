...
TREES <- 1:99 # TODO 1:100 this once we've generated the missing trees!
...
The only analyses to produce significantly different results from implied weights
(at $k = 3$) or Bayesian are:

```{R k significance, echo=FALSE}
...
```{R Partition divergence - best}
apply(rf_dists, 2, which.min) - 1
```

Do different values of k produce significantly different results?
