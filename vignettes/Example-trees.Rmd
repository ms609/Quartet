...
TernaryQuarts<-function(Func=Quartet2Ternary, zoom=1, padding=0.1) {
  ...
  JoinTheDots(Func('implied3'), col=COL_3,  pch=PCH_IW, cex=1.1)
...
```{R CL trees - quartets, fig.height=4.5, fig.width=9, echo=FALSE}
TREES <- 1:98 # TODO 1:100 this once we've generated the missing trees!
# # There is essentially no difference if we exclude datasets with a low CI.
# # The lines just slide up a little.
# data(clCI)
# cis <- as.double(clCI[-c(36, 67)])
# TREES <- TREES[cis > 0.26]

par(mfrow=c(1, 2), mar=rep(0.3, 4))
AverageQuarts <- function (item) apply(clQuartets[[item]][c('s', 'd', 'r2'), , TREES], 2, rowMeans)
TernaryQuarts(AverageQuarts)
AddLegend()
TernaryQuarts(Func=AverageQuarts, zoom=2.25, padding=0.01)

```

And partitions for comparison:

```{R CL trees - partitions, fig.height=6, fig.width=6, echo=FALSE}
...
JoinTheDots(AverageSplits('implied10'), col=COL10, pch=PCH_IW, cex=1.1)
JoinTheDots(AverageSplits('implied5'), col=COL_5, pch=PCH_IW, cex=1.1)
JoinTheDots(AverageSplits('implied3'), col=COL_3, pch=PCH_IW, cex=1.1)
JoinTheDots(AverageSplits('implied2'), col=COL_2, pch=PCH_IW, cex=1.1)
...
text  (sqrt(3/4) * 1.01, 0.30, pos=3, 'Increasing RF distance', cex=0.8, srt=270)
legend('bottomright', cex=0.8, bty='n',
       pch=c(PCH_MK, PCH_EQ, rep(PCH_IW, 6)), pt.cex=1.1,
       col=c(COL_MK, COL_EQ, COL10, COL_5, COL_3, COL_2, COL_1),
       legend=c('Markov', 'Equal weights', paste0('Implied, k=', c(10, 5, 3, 2, 1)))
)
```
