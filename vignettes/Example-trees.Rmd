...
As such, given a pair of characters, a reconstruction that assigns infinitely many changes of one character (but no additional steps to the other) will be preferred to one in which both characters undertake a single additional step.  The emphasis here is thus on maximising the number of convex characters (_cf._ clique analysis), rather than minimizing the total number of steps on the tree (_cf._ parsimony analysis).
...
[_cf._ @Smith2015;@Zhang2016] seems to be justified.
