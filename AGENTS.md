...
2026-03-18 | — | R-level preprocessing optimization | Three R-level changes: (1) `ResolvedQuartets` children list construction O(n²)→O(n) via `split()` instead of `lapply(nodes, function(node) child[parent==node])`; extracted core into `.resolvedQuartetsCounts()` that skips redundant `.CheckSize()`/`Preorder()`. (2) `TwoListQuartetAgreement` now preprocesses `trees1` once (Preorder + ResolvedQuartets) instead of N₂ times. (3) `QuartetStatus` nTip path: moved invariant `ResolvedQuartets(cf)` and `TipLabels(cf)` outside vapply loop. One-to-many at 400 tips: **0.14s → 0.082s (1.7×)**. FAIL 15 (pre-existing), PASS 332. Benchmark: `inst/benchmarks/post-opt-R-preprocess-2026-03-18.rds`.
