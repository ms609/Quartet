...
#' 
#' @return A vector of three integers, listing the number of splits that [1] are
#'         present in both trees; [2] are present in \code{x} but not \code{cf};
#'         [3] are present in \code{cf} but not \code{x}.
#' @author Martin R. Smith
#' @export
CompareSplits <- function (x, cf) {
  ...
  c(common, length(x_hashes) - common, length(cf_hashes) - common)
}
...
MatchingSplits <- function (trees) {
  ...
  trees <- lapply(trees, RenumberTips, tipOrder = tree1Labels)
  splits <- lapply(trees, Tree2Splits)
  vapply(splits, CompareSplits, cf=splits[[1]], double(2))
